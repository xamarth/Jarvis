...
async def _callback_check(event):
    ...
    if not _jar_cache.get("admin_callback"):
        _jar_cache.update({"admin_callback": {id_: None}})
    else:
        _jar_cache["admin_callback"].update({id_: None})
    while not _jar_cache["admin_callback"].get(id_):
        await asyncio.sleep(1)
    key = _jar_cache.get("admin_callback", {}).get(id_)
    del _jar_cache["admin_callback"][id_]
    return key


async def get_update_linked_chat(event):
    if _jar_cache.get("LINKED_CHATS") and _jar_cache["LINKED_CHATS"].get(event.chat_id):
        _ignore = _jar_cache["LINKED_CHATS"][event.chat_id]["linked_chat"]
    else:
        channel = await event.client(
            functions.channels.GetFullChannelRequest(event.chat_id)
        )
        _ignore = channel.full_chat.linked_chat_id
        if _jar_cache.get("LINKED_CHATS"):
            _jar_cache["LINKED_CHATS"].update({event.chat_id: {"linked_chat": _ignore}})
        else:
            _jar_cache.update(
